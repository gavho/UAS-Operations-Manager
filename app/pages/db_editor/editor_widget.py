...
class DBEditorWidget(QWidget):
    def __init__(self):
        ...
        db_manager.database_added.connect(self.populate_db_tree)
        db_manager.database_removed.connect(self.populate_db_tree)
    ...
    def on_tree_item_selected(self, current_item, previous_item):
        ...
        if item_data and item_data.get('type') == 'table':
            ...
            connection_info = db_manager.get_connection(db_path)
            if connection_info:
                self.db_path = db_path
                self.current_table_name = table_name
                models.DB_SESSION = connection_info['session']
                models.DB_MODELS = connection_info['models']
    ...
    def open_db_dialog(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Open Database File", "", "SQLite Database Files (*.db *.sqlite)")
        if fname:
            if not db_manager.add_database(fname):
    ...
    def populate_db_tree(self):
        ...
        self.db_tree_widget.setHeaderLabels([f'Databases ({len(db_manager.get_all_connections())})'])
        all_connections = db_manager.get_all_connections()
        for path, connection_info in all_connections.items():
            db_name = os.path.basename(path)
            db_node = QTreeWidgetItem(self.db_tree_widget, [db_name])
            db_node.setData(0, Qt.UserRole, {'path': path, 'type': 'db'})
            models_dict = connection_info.get('models', {})
            for table_name in sorted(models_dict.keys()):
                table_node = QTreeWidgetItem(db_node, [table_name])
                table_node.setData(0, Qt.UserRole, {'path': path, 'table': table_name, 'type': 'table'})
    ...
    def open_tree_menu(self, position):
        ...
        if item_data.get('type') == 'db':
            close_action = menu.addAction("Close Database")
            action = menu.exec_(self.db_tree_widget.mapToGlobal(position))
            if action == close_action:
                db_manager.remove_database(item_data['path'])
                self.table_view.clear()
                self.table_view.setRowCount(0)
                self.table_view.setColumnCount(0)
    ...
    def paste_from_clipboard(self):
        ...
        self.table_view.itemChanged.disconnect()
        headers = [c.key for c in models.DB_MODELS[self.current_table_name].__table__.columns]
        try:
